...
existing gsim/FIR bootstrap path
  -> profile current MT runtime
  -> reduce scheduler / worker / merge overhead
  -> expand bounded RepCut-lite where correctness can be proven
  -> run XiangShan bootstrap gsim path correctness/performance matrix
```
...
3. 执行 08P：profile 当前 `off`、`seq`、`buffered-seq`、`mt`、`mt + repcut-lite` 路径，先找出瓶颈。
4. 执行 09P：只保留有 A/B 证据的 MT runtime throughput 优化。
5. 执行 10P：扩展 RepCut-lite 的实用范围，同时保留预算、fallback 和 trace correctness。
6. 执行 11P：在 XiangShan bootstrap gsim path 上做 tiny-cycle correctness smoke，再做 `GSIM_THREADS=1/2/4` 性能矩阵。

短期不建议恢复 Wolvrix 前端工作或直接跳到大型 XiangShan 长跑性能测试。这样会把 frontend、runtime、调度、正确性、性能瓶颈混在一起，定位成本很高。
...
4. 如果要继续开发 08P-11P，再看 `gsim-mt-minimal-plan.md` 和 performance-first 子计划。
