...
def fail(message):
    print(f"mt-16x-active-frequency-gate failed: {message}", file=sys.stderr)
    sys.exit(1)
...
def check_structure(model_dir):
    ...
    expect("nextActive.orWord" in text, "generated MT model has no ActivationDelta::orWord calls")
...
def main():
    ...
    for thread_count in (1, 2, 4):
        ...
        expect(seq_result.stdout == mt_result.stdout,
               f"MT active-frequency trace differs from seq trace at GSIM_THREADS={thread_count}")
        main_values = parse_key_values(profile_line(mt_result.stderr, "[mt-profile] helper_mode="))
        delta_values = parse_key_values(profile_line(mt_result.stderr, "[mt-profile] activation_delta"))
    ...
    summary = ["mt-16x-active-frequency-gate ok: seq_mt_trace=matched"]
