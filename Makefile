...
check-mt-22x-macro-fiber-planner: $(GSIM_BIN)
	python3 scripts/check-mt-22x-macro-fiber-planner.py
	python3 scripts/check-mt-22x-macro-fiber-fixtures.py \
		--gsim-bin "$(GSIM_BIN)" \
		--input-dir "$(FIR_TEST_INPUT_DIR)" \
		--work-dir "$(BUILD_DIR)/mt-22x-macro-fiber-fixtures" \
		--report-dir "$(BUILD_DIR)/mt-22x-macro-fiber-fixtures/reports" \
		--timeout "$(if $(strip $(FIR_TEST_TIMEOUT)),$(patsubst %s,%,$(FIR_TEST_TIMEOUT)),30)"

check-mt-23x-blocker-inspection: $(GSIM_BIN)
...
check-mt-active-buffer: $(GSIM_BIN)
	...
	@test -f "$(FIR_TEST_INPUT_DIR)/$(FIR_TEST).fir" || \
		(echo "Missing FIR test: $(FIR_TEST_INPUT_DIR)/$(FIR_TEST).fir" >&2; exit 1)
	@rm -rf "$(FIR_TEST_OUTPUT_DIR)/$(FIR_TEST)-helper-seq" \
		"$(FIR_TEST_OUTPUT_DIR)/$(FIR_TEST)-buffered-seq"
	@mkdir -p "$(FIR_TEST_OUTPUT_DIR)/$(FIR_TEST)-helper-seq" \
		"$(FIR_TEST_OUTPUT_DIR)/$(FIR_TEST)-buffered-seq"
	set -o pipefail && $(TIME) $(FIR_TEST_TIMEOUT_PREFIX) $(GSIM_BIN) --dir "$(FIR_TEST_OUTPUT_DIR)/$(FIR_TEST)-helper-seq" \
		$(GSIM_FLAGS_EXTRA) --mt-helper-mode=seq "$(FIR_TEST_INPUT_DIR)/$(FIR_TEST).fir" | tee "$(FIR_TEST_OUTPUT_DIR)/$(FIR_TEST)-helper-seq/gsim.log"
	set -o pipefail && $(TIME) $(FIR_TEST_TIMEOUT_PREFIX) $(GSIM_BIN) --dir "$(FIR_TEST_OUTPUT_DIR)/$(FIR_TEST)-buffered-seq" \
		$(GSIM_FLAGS_EXTRA) --mt-helper-mode=buffered-seq "$(FIR_TEST_INPUT_DIR)/$(FIR_TEST).fir" | tee "$(FIR_TEST_OUTPUT_DIR)/$(FIR_TEST)-buffered-seq/gsim.log"
	python3 scripts/check-mt-active-buffer.py "$(FIR_TEST_OUTPUT_DIR)/$(FIR_TEST)-helper-seq" "$(FIR_TEST_OUTPUT_DIR)/$(FIR_TEST)-buffered-seq"
	python3 scripts/check-mt-helper-equivalence.py "$(FIR_TEST_OUTPUT_DIR)/$(FIR_TEST)-helper-seq" "$(FIR_TEST_OUTPUT_DIR)/$(FIR_TEST)-buffered-seq"

run-mt-fir-smoke: $(GSIM_BIN)
	python3 scripts/run-mt-fir-smoke.py \
		--gsim-bin "$(GSIM_BIN)" \
		--input-dir "$(FIR_TEST_INPUT_DIR)" \
		--output-dir "$(FIR_TEST_OUTPUT_DIR)/mt-smoke" \
		--cases "$(MT_FIR_SMOKE_CASES)" \
		"--flags=$(MT_FIR_SMOKE_GSIM_FLAGS) $(GSIM_FLAGS_EXTRA)" \
		--timeout "$(FIR_TEST_TIMEOUT)"
...
.PHONY: run-fir-test check-mt-schedule-json check-mt-22x-macro-fiber-planner check-mt-23x-blocker-inspection check-mt-active-buffer run-mt-fir-smoke fir-tests
